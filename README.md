...
This report documents the compromise of the Jangow01 machine from initial enumeration to full root access.
The attack was performed from a Kali Linux machine in a controlled lab environment.
...
To identify active hosts within the network, scanning was performed to confirm the target system availability.

Port Scanning

A port scan was conducted to identify exposed services.
...
While navigating the web application:
...
Instead of relying on common misconfigurations (like SUID or sudo), a kernel exploit path was chosen because:

It provided a direct escalation route
The system version matched known vulnerable kernels

The exploit was executed on the target machine, resulting in:
...
What Could Have Been Improved (Defensive View)
...
This report is based on a controlled CTF/lab scenario and reflects a structured attack methodology rather than automated exploitation.
